...
crop::dev.off.crop(file = "figure/cover.pdf")
```
...
for(i in seq(length(aligned_plots))) {
  ...
  crop::dev.off.crop(file = paste0("figure/austa",i,".pdf"))
}
...
crop::dev.off.crop(file = paste0("figure/austa10.pdf"))
```
...
crop::dev.off.crop(file = paste0(here::here("figure"), "/elecscatter1.png"))
png(paste0(here::here("figure"), "/elecscatter2.png"),
    width = 20, height = 10, units="cm", res=300, type="cairo-png")
print(aligned_plots[[2]])
crop::dev.off.crop(file = paste0(here::here("figure"), "/elecscatter2.png"))
```
...
for (i in seq_along(states)) {
  ...
  p <- df |>
    ...
    labs(y = "Demand (GW)", x = "Temperature (degrees C)")
  print(p)
  crop::dev.off.crop(file = paste0(here::here("figure"), "/elec", i, ".pdf"))
}
```
...
for (i in seq_along(origins)) {
  ...
  print(p)
  crop::dev.off.crop(file = paste0(here::here("figure"), "/ensemble", i, ".pdf"))
}
```
...
#| warning: false
#| include: false
# PDF graph for gamma distribution
actual <- 3
prob <- c(0.1, 0.3, 0.5, 0.7, 0.9)
np <- length(prob)
qactual <- qgamma(prob, 2)
qpactual <- qp(prob, actual)
plist <- vector("list", np+2)
labels <- paste(sprintf("%.2f", prob[i]), "quantile")
...
Cairo::CairoPDF("figure/pdf00.pdf", width = 15 / 1.5, height = 7.5 / 1.5)
print(p / plot_spacer())
crop::dev.off.crop(file = paste0("figure/pdf00", i, ".pdf"))

p <- p +
  geom_point(aes(x = actual, y = y), data = tibble(actual = actual, y = 0)) +
  geom_label(aes(x = actual, y = 0.02, label = "Actual"))

Cairo::CairoPDF("figure/pdf0.pdf", width = 15 / 1.5, height = 7.5 / 1.5)
print(p /plot_spacer())
crop::dev.off.crop(file = paste0("figure/pdf0", i, ".pdf"))
```
...
for (i in seq_along(prob)) {
  ...
  p2 <- tibble(
    ...
    coord_cartesian(ylim = c(0, 2), xlim = c(0, 10), expand = FALSE, clip = "off")
  Cairo::CairoPDF(paste0("figure/pdf", i, ".pdf"), width = 15 / 1.5, height = 7.5 / 1.5)
  print(p1 / plot_spacer())
  crop::dev.off.crop(file = paste0("figure/pdf", i, ".pdf"))
  Cairo::CairoPDF(paste0("figure/pdfqs", i, ".pdf"), width = 15 / 1.5, height = 7.5 / 1.5)
  print(p1 / p2)
  crop::dev.off.crop(file = paste0("figure/pdfqs", i, ".pdf"))
}
```

\only<1>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdf00}}
\only<2>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdf0}}
\only<3>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdf5}}
\only<4>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdf4}}
\only<5>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdf3}}
\only<6>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdf2}}
\only<7>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdf1}}
\only<8>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdfqs1}}
\only<9>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdfqs2}}
\only<10>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdfqs3}}
\only<11>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdfqs4}}
\only<12>{\placefig{0.2}{1.4}{width=15.7cm,height=20cm}{pdfqs5}}
...
for (i in seq_along(prob)) {
  ...
  p2 <- tibble(
    ...
    geom_label(aes(x = qactual, y = 2, label = paste(sprintf("%.3f", prob[i]), "quantile")), col = "blue") +
    coord_cartesian(ylim = c(0, 2), xlim = c(0, 10), expand = FALSE, clip = "off")
  ...
  p3 <- p3data |>
    ...
    coord_cartesian(ylim = c(0, 2), xlim = c(0, 1), expand = FALSE, clip = "off")

  Cairo::CairoPDF(file = paste0("figure/qs", i, ".pdf"), width = 15 / 1.5, height = 7.5 / 1.5)
  print((plot_spacer() | p1) / (p3 | p2))
  crop::dev.off.crop(file = paste0("figure/qs", i, ".pdf"))
}
```
