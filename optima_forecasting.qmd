...
* Later case numbers are uncertain and incomplete as date of onset is not known until symptoms show and a test is obtained.
...
\includegraphics[width=14.5cm]{austa10}
\only<2>{
\begin{textblock}{7.3}(0.9,1.6)\begin{alertblock}{}\fontsize{12}{13}\sf
\begin{itemize}\itemsep = 0.2cm
\item A 90\% prediction interval will contain the actual values 90\% of the time.
\item A good prediction interval will be as narrow as possible.
\item We want these properties for all possible prediction intervals.
\item A probability score is a measure across all possible prediction intervals that balances calibration (coverage) with sharpness (narrow intervals).
\end{itemize}
\end{alertblock}
\end{textblock}
}
...
\begin{textblock}{14.5}(0.5,8)\fontsize{13}{15}\sf
\begin{block}{}
