...
pbs %>%
  ...
  model(ETS(Cost)) %>%
  forecast(h = "3 years") %>%
  autoplot(pbs, level=c(50,60,70,80,90))
```
...
\only<2>{\full{ensemble2}}
\only<3>{\full{ensemble3}}
\only<4>{\full{ensemble4}}
\only<5>{\full{ensemble5}}
\only<6>{\full{ensemble6}}
\only<7>{\full{ensemble7}}

<!-- \centerline{\animategraphics[controls,buttonsize=0.3cm,width=14cm]{5}{figs/ensemble}{1}{107}} -->

# Are we uncertain enough?
...
for(i in seq_along(prob)) {
  ...
  p2 <- tibble(
      ...
      ggplot(aes(x=y, y=Q)) +
      geom_line() +
      labs(x=latex2exp::TeX("Quantile $q_{p}$"),
           y = latex2exp::TeX("Quantile score $S_{p}$")
      ) +
      geom_line(aes(x=y, y=Q), col='red',linetype=2,
...
for(i in seq_along(prob)) {
  ...
  p3 <- p3data  %>%
      ...
      labs(x=latex2exp::TeX("Probability p"),
         y = latex2exp::TeX("Quantile score $S_{p}$")
      ) +
