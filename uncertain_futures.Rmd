...
for(i in seq_along(prob)) {
  ...
  p1 <- gamma %>%
    ...
    labs(y="Probability density time t", x=latex2exp::TeX("Quantile $f_{p,t}$")) +
  ...
  p2 <- tibble(
    ...
    ggplot(aes(x=y, y=Q)) +
    geom_line() +
    labs(x=latex2exp::TeX("Quantile $f_{p,t}$"),
         y = latex2exp::TeX("Quantile score $Q_{p,t}$")
    ) +
    ...
    geom_label(aes(x=qactual,y=11.6, label=paste(sprintf("%.2f",prob[i]), "quantile")), col='blue') +
  ...
  p3 <- tibble(
      ...
      geom_line(data=tibble(p=rep(prob[i],2), Q=c(0,qpactual)), col='blue', linetype=2) +
      geom_label(aes(x=prob[i], y=-1, label="0.80"), col='blue') +
...
\begin{textblock}{15}(0.3,1.4)
\animategraphics{99}{figure/qs}{1}{99}
\end{textblock}
