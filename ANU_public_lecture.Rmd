...
localcases <- readr::read_csv("local_cases_input.csv") %>%
  rename(
    n = count,
    date = date_onset
  ) %>%
  # Adjust count to include cases yet to be detected
  mutate(n = n / case_ascertainment_probability) %>%
  # Remove last days with prob of detection < 0.5
  #filter(case_ascertainment_probability > 0.5) %>%
...
```{r crps_plot, fig.height=4.6, fig.width=10, eval=FALSE}
crps %>%
  filter(
    h >= 1, h <= 20,
...
###
Slides: \texttt{\href{https://robjhyndman.com/seminars/anu2022}{robjhyndman.com/seminars/anu2022}}
